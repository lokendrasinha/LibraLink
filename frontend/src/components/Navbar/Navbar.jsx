  ...
  return (
    ...
    <nav className='z-50 relative flex bg-zinc-800 text-white px-8 py-4 items-center  justify-between'>
      <Link to="/" className='flex items-center'>
        ...
        <h1 className='text-2xl font-semibold'>LibraLink</h1>
        </Link>
      <div>
        <div className='nav-links-libralink block md:flex items-center gap-4'>
     ...
     <button className=' block md:hidden text-white text-2xl hover:text-zinc-400 transition-all duration-300' onClick={()=>setMobileNav(MobileNav === "hidden" ? "block" : "hidden")}><LuMenu /></button>
        </div>
      </div>
    </nav>
    ...
    {links.map((items,i)=>(
              <Link 
              to={items.link}
              className={`${MobileNav} text-white text-4xl mb-4 font-semibold hover:text-blue-500 transition-all duration-300`} key={i}
              onClick={()=>
        ...
        <Link to="/LogIn" className={`${MobileNav} px-8 mb-8 text-3xl font-semibold py-2 border border-blue-500 rounded hover:bg-white text-white hover:text-zinc-800 transition-all duration-300`}>LogIn</Link>
